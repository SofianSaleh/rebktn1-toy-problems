...
BinaryHeap.prototype.getRoot = function (index) {
    // const index = this._heap.indexOf(element)
    const parentIndex = Math.floor((index - 1) / 2)

    return this._heap[parentIndex]
...
BinaryHeap.prototype.insert = function (value) {
    ...
    if (this._heap.length === 0) {
        ...
        const root = this.getRoot(this._heap.length)

        this._heap.push(value)
        if (!this._compare(root, value)) {
            [this._heap[this._heap.indexOf(root)], this._heap[this._heap.length - 1]] = [this._heap[this._heap.length - 1], this._heap[this._heap.indexOf(root)]]
        }
    }
    return this._heap
...
BinaryHeap.prototype.removeRoot = function () {
    // TODO: Your code here
    if (this._heap.length === 0) return (` Heap is empty`)
    const root = this.getRoot(this._heap.length - 1)
    let childrenIndices = this.getChildren(root)




    if (this._compare(this._heap[childrenIndices[0]], this._heap[childrenIndices[1]])) {
        [this._heap[this._heap.indexOf(root)], this._heap[childrenIndices[0]]] = [this._heap[childrenIndices[0]], this._heap[this._heap.indexOf(root)]]

    } else {
        [this._heap[this._heap.indexOf(root)], this._heap[childrenIndices[1]]] = [this._heap[childrenIndices[1]], this._heap[this._heap.indexOf(root)]]
...
BinaryHeap.prototype.getChildren = function (root) {
    // TODO: Your code here
    let rootIndex = this._heap.indexOf(root)
    let childrenIndices = [rootIndex * 2, rootIndex * 2 + 1]
    return childrenIndices

}

var x = new BinaryHeap()
x.insert(0)
x.insert(1)
x.insert(2)
x.insert(8)

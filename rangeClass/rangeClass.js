...
var Range = function (start, end, step = 1) {
    // we declare the start , end and the step 
    this.start = start;
    this.end = end;
    this.step = step;
    this.result = false;
...
Range.prototype.size = function (val = null) {
    // we find the size by looping through the range  and checking
    var count = 0;
    // if the end is undefined we shall return 1
    if (this.end === undefined) {
        return 1
    }
    // if the step is undefined we shall set its value to 1
    if (this.step === undefined) {
        this.step = 1;
    }
    // now we loop
    for (var i = this.start; i <= this.end; i += this.step) {
        //this line wont work unless it is invoke from the includes function
        if (val !== null) {
            if (val === i) {
                this.result = true;               
            }
...
Range.prototype.each = function (callback) {
    // we loop and apply the callback to each element
    for (var i = this.start; i <= this.end; i += this.step) {
        callback(i);
    }
};
...
Range.prototype.includes = function (val) {
    // here we invoke the size and send it the value that we want to compare
